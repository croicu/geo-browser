...
- **Widget**: area-scoped `TileCacheControl` (topright, alongside `SearchControl`/`ImageOverlayWidget` — see Map Control Positions below), VCR-style: hollow red circle (idle, tap to start recording) / filled red square (recording, tap to stop). A second, separate button (trash icon) clears the *current area's* cached tiles via `TileCacheStore.clear()` (`caches.delete()` on that area's named cache only, `MapLayerFlyoutHandle.clearTileCache(areaId)`) — matches recording's own per-area scoping.
...
All interactive controls are at `topright` (stacked top-to-bottom in render order). `MapLayerFlyoutControl` is session-level (`MapView`, always present); `SearchControl`, `TileCacheControl`, and `ImageOverlayWidget` are area-scoped (`CurrentAreaBundle`, only exist while a current area is attached):
...
TileCacheControl      (offline tile-cache tap-to-download widget — current area only)
