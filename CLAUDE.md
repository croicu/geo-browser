...
`Logger.perf(description, elapsedSeconds)` is a duration marker for a timing-sensitive span (a network call, a slow computation, anything worth measuring) — ported from the sibling `tpl-py`/`geo-builder` ecosystem's own `Logger.perf()`. Unlike every other `Logger` method, the category isn't the caller's choice: it's always logged at `info` under the fixed `LogCategory.Perf` (`"perf"`), so every duration marker in the app — regardless of which component logged it — is filterable together via `?logCategory=perf`, separate from that component's own regular logging. Message format: `"duration: {elapsed:.3f}s - {description}"`.

```ts
const start = performance.now();
// ... the thing being measured ...
getLogger().perf("some_span", (performance.now() - start) / 1000);
```

**Don't call this per-tile/per-item inside a loop that can run many times.** The first version of `src/tiles/tileFetcher.ts`'s tile-fetch timing did exactly that (a `.perf()` call per tile) and it was a real mistake, not just noise to filter — one log line per tile produced thousands of fragmented entries with no way to actually get a sense of overall performance from them, and `?logCategoryExclude=perf` just hides the problem rather than fixing the tool. The fix, still in place (`TileFetcher`'s `getStats()`/`resetStats()`): accumulate count/hit-miss/min/avg/max internally with no logging per call, and have the *bounded-span owner* (whatever code represents one logical operation with a real start and end) log a single aggregate summary at the end via `getLogger().info("...summary", statsObject, LogCategory.Perf)` — richer than a bare `.perf()` duration since it can carry a whole stats payload as `props`, and actually answers "how did this go" at a glance. Reach for a raw `.perf()` call only when the span truly has just one instance per invocation (a single network request, a one-shot computation) — reach for accumulate-then-summarize whenever the same span repeats many times within one logical operation.
...
`src/maps/tileProvider.ts` holds the `TileProvider` interface, `osmTileProvider` and `cartoTileProvider` constants, and the module-level `getActiveTileProvider`/`setActiveTileProvider` store (survives map recreations). OSM is the default (the cached, offline-capable provider — see Offline Tile Caching below); Carto is online-only, never cached. OSM tiles use the `dark-osm` CSS filter; CARTO tiles do not.

### Offline Tile Caching

Record-while-browsing map tile caching, so a network drop doesn't blank the map background even though POI GeoJSON already survives one. Full design and history (including why the original bulk-download design was scrapped): [tasks/tile_caching.md](tasks/tile_caching.md), tracked as [geo-browser#103](https://github.com/croicu/geo-browser/issues/103).

**Not a bulk pre-fetch feature.** The original design (tap-to-cache an entire area ahead of time, rate-limited) was deleted mid-implementation after confirming — against the actual policy source repo's commit history, not just current wording — that OSM's tile usage policy has explicitly prohibited "download for offline use"/bulk pre-fetching for years, regardless of rate limit. No amount of throttling was ever going to make that design compliant. See `tasks/tile_caching.md`'s Design history section for the full arc.

What's actually implemented instead:

- **Provider split**: OSM is the only cached provider; Carto stays online-only and is disabled in the flyout while offline (`MapLayerFlyoutControl`'s `updateTileButtons`/`onNetworkStatusChange`, `src/runtime/networkStatus.ts`). Going offline while Carto is active force-switches to OSM. Caching Carto Voyager too was considered and declined for an unrelated reason — its raster tiles now need an API key this repo doesn't have (confirmed live: "API KEY REQUIRED" watermark) and Carto is retiring raster basemaps outright; see [geo-browser#104](https://github.com/croicu/geo-browser/issues/104).
- **Record-while-browsing, not pre-fetch**: recording just flips write-through on for whatever tiles Leaflet's own ordinary viewport-driven loading already requests — the ordinary "modest, short-range look-ahead" every map view does, and precisely OSM's own explicitly *permitted* pattern. Nothing is fetched that isn't currently being viewed. Not area-bbox-bounded — recording follows wherever the viewport goes while it's on.
- **No persistence of the recording toggle itself.** `CurrentAreaBundle` tracks it as a plain in-memory `_tileCacheRecording` boolean, reset to `false` on every `attach()`/`hide()`/`destroy()` — it's a deliberate, in-the-moment action, not a resumable background job. What persists is only the tiles already written into the Cache API.
- **No eviction/expiry of its own.** `CacheApiTileCacheStore` has no TTL, size cap, or LRU pruning — a written tile stays until the "clear cache" button wipes that area's cache outright, or the browser's own storage-pressure/inactivity eviction takes the whole origin (see `src/runtime/storagePersistence.ts` below). Deliberate: map tile content never goes stale, so there's nothing to invalidate on a schedule; unbounded growth over many recorded areas is an accepted tradeoff rather than engineered around, since measured real-world usage stays proportional to how much was actually browsed (confirmed live: a real Seattle recording session was ~25MB, nowhere near any realistic quota).
- **Requests persistent storage when recording starts** (`requestPersistentStorage()`, `src/runtime/storagePersistence.ts`) — `navigator.storage.persist()`, fire-and-forget, guarded on the API existing. Protects against the browser evicting a just-recorded area's cache before it's ever revisited (e.g. recording a trip's cities weeks before departure). On iOS this mainly succeeds for a Home Screen install, not a plain Safari tab — WebKit's own storage policy grants persistent mode mainly to Home Screen apps.
- **Per-area cache**: one named Cache API cache per area, `geo-browser.tiles.<areaId>` (`CacheApiTileCacheStore`, `src/runtime/cacheApiTileCacheStore.ts`, constructed with the area id) — `leafletFactories.ts` keeps a `Map<string, TileFetcher>` (`getTileFetcherForArea()`) so each area gets its own lazily-created `TileFetcher`/store pair instead of one shared global cache. Which area's tiles a tile is written into is decided by whichever area is current when the fetch happens, not the tile's own geography — a tile near an area boundary is attributed to whichever area you were browsing it under. No separate metadata store is needed at all.
- **Three tile-layer classes, picked by `buildTileLayer()`**: a plain `L.tileLayer` when there's no current area at all (nothing to attribute a cache to); `OfflineFallbackTileLayer` for ordinary browsing (recording off, `?debug` off — the common case); `CachingTileLayer` while recording or under `?debug`. The latter two are both area-scoped (own a per-area `TileFetcher`, see `getTileFetcherForArea()` below) — `MapLayerFlyoutControl.setTileCacheEnabled()` rebuilds the tile layer whenever `shouldRebuildTileLayer()` (`tiles/tileCacheDecision.ts`) says the recording flag flipped *or* the current area changed, since both area-scoped classes need to swap to a different area's `TileFetcher`.
- **`CachingTileLayer`** overrides `createTile()` to fetch manually (not via `<img src>`) so it can check the Cache API first and, while recording, write a live fetch back into the cache. Always computes the tile URL via `tiles/tileUrl.ts`'s `buildTileUrl()` (fixed first subdomain, sacrificing Leaflet's `{s}`-rotation connection-parallelism) so the same tile always maps to the same cache key regardless of how many times it's requested. Constructed whenever `shouldUseCachingLayer(recording, debug)` (`src/tiles/tileCacheDecision.ts`, Leaflet-free/unit-tested) says so.
- **`OfflineFallbackTileLayer` fixes a real gap that shipped with the original design**: recording writes tiles into the cache, but nothing ever read them back during *ordinary* browsing — `buildTileLayer()` used to fall back to a plain `L.tileLayer` whenever caching/debug wasn't active, and a plain tile layer never touches the Cache API in either direction. Confirmed live: reopening the app fully offline (recording off, no `?debug`, the actual way the app gets used day to day) showed a completely blank map background despite tiles genuinely being cached from an earlier recording session — nothing was ever asking for them. **Cache-first, not fetch-first-with-fallback**: `createTile()` always checks `TileFetcher.tryCache()` before doing anything else — a cache hit displays immediately with zero network involvement, regardless of connection speed; only a genuine miss falls through to a plain native `<img src>` (Leaflet's own `{s}`-rotating `getTileUrl()`, full connection-parallelism, best-effort — no special handling, a failed load here just doesn't show, like any ordinary tile layer with no cache). This also covers slow/flaky connections, not just fully offline — an earlier version only consulted the cache on an `onerror`, which never triggers for a merely-slow load that eventually succeeds, so a cached tile on a bad connection got no benefit at all until the explicit ask to make cache-first the rule: "if a tile is in the cache we should always display it, fetching from OSM is always best effort." Never writes to the cache; write-through stays exclusively tied to explicit recording.
- **`?debug` still reads the cache even with recording off**, via `CachingTileLayer` specifically (not `OfflineFallbackTileLayer`) — `shouldUseCachingLayer(recording, debug)` returns true under `?debug` alone, constructing `CachingTileLayer` with `writeThrough: false`, so the debug-hit marking below has something to show while just browsing an area recorded in an *earlier* session.
- **Parent-tile placeholder on a genuine cache miss, shared by both `OfflineFallbackTileLayer` and `CachingTileLayer`**: instead of a blank rectangle while the real tile loads, a genuine miss looks for an already-cached tile at a coarser zoom (the same technique Google Maps and others use) — crops the matching quadrant and scales it up via `<canvas>`, shows that immediately, then silently swaps in the real tile once it loads (no second call to Leaflet's `done()` — an in-place image swap, not a second tile load as far as Leaflet is concerned). `findParentPlaceholder()`/`cropAndScale()` are module-level free functions (not methods) specifically so both tile-layer classes can share them without a common base class — free functions were chosen deliberately here over a shared abstract base, since each class's "how to finally display the real tile" differs enough (write-through + debug-rectangle marking in `CachingTileLayer` vs. a plain native `<img>` load in `OfflineFallbackTileLayer`) that forcing a shared hierarchy seemed more confusing than two similar-but-distinct `createTile()`s calling the same two helpers. `CachingTileLayer` races the placeholder against its real `fetchTile()` call (which still handles write-through regardless of which one wins); `OfflineFallbackTileLayer` only tries a placeholder after confirming a miss via `tryCache()`, then a plain best-effort native load if no placeholder exists either. Walks parent zoom levels closest-first via `computeParentTileCrop()` (`src/tiles/tileParent.ts`, pure/unit-tested — the quadrant math, not the canvas drawing), stopping at `AreaRenderClassifier.MIN_LOADED_ZOOM` (reused rather than inventing a second "how far is too far" threshold). This was originally built only in `OfflineFallbackTileLayer` and confirmed live to have zero effect under `?debug` — debug mode uses `CachingTileLayer` instead, a completely separate class the placeholder logic hadn't been added to yet.
- **Stale `img.onload`/`onerror` handler bug, fixed after live testing**: `upgradeToLiveTile()` (the "swap in the real tile once it loads" step) originally reassigned `img.src` a second time without first clearing the placeholder's own `onload`/`onerror` handlers — so the OLD handler fired again on the new load, calling Leaflet's `done()` a second time for the same tile (a contract violation) and recursively re-triggering the upgrade itself, a runaway loop that never settled. Confirmed live as the cause of two distinct-looking symptoms that were actually the same bug: a blank tile flashing in over a placeholder after a zoom transition, and a placeholder staying stuck indefinitely instead of upgrading even once the network came back. Fixed by explicitly setting `img.onload = null; img.onerror = null;` before the final reassignment in both `OfflineFallbackTileLayer.upgradeToLiveTile()` and `CachingTileLayer.resolveLiveWithPlaceholder()`'s equivalent "already shown, silently upgrade" branch — the latter was already clearing `onload` but not `onerror`, the same bug class triggered by a subsequent failure instead of a subsequent success.
- **Widget**: `ToolsFlyoutControl` (topright, area-scoped — see Map Control Positions below), a trigger-button-opens-panel flyout (mirroring `MapLayerFlyoutControl`'s own pattern) consolidating record/stop (VCR-style row: hollow circle icon idle, filled square icon recording), clear cache (trash icon, wipes the *current area's* cached tiles via `TileCacheStore.clear()` — `caches.delete()` on that area's named cache only, `MapLayerFlyoutHandle.clearTileCache(areaId)`, matching recording's own per-area scoping), and the **paste-image trigger** (calls `ImageOverlayWidget.triggerPaste()` — only the trigger moved here, the actual adjustment toolbar that appears once an image loads stays entirely in `ImageOverlayWidget`, untouched). Debug-only rows (`?debug`) load a fixed Google/Apple Maps test image via `ImageOverlayWidget.loadGoogleMapsDebugImage()`/`loadAppleMapsDebugImage()`. Replaced three separate always-visible topright controls (record/stop, clear, and `ImageOverlayWidget`'s own paste button) after confirming live that stacking every action as its own floating square button got crowded.
- **`Logger.perf()` and `TileFetcher.getStats()` survive the redesign** as general-purpose infrastructure (see Perf Logging above) even though the bulk job they were built to diagnose is gone — `getStats()`'s cache-hit/live-fetch/min/avg/max accumulation is still exactly what you'd want for diagnosing tile-loading slowness during a recording session, just with no `TilePrefetchJob` left to call `resetStats()`/log a summary automatically.
- **Debug-only cache-hit marking is a real Leaflet vector layer, not CSS on the tile `<img>`.** The first version applied a `.tile-cache-hit-debug` CSS class (inset `box-shadow`) directly to cache-hit tile images — abandoned after confirming live it just never painted: `document.querySelectorAll('.tile-cache-hit-debug').length` matched `document.querySelectorAll('img.leaflet-tile').length` exactly (100% of visible tiles were hits) and `getComputedStyle()` showed the shadow resolved correctly, yet nothing rendered on screen — never root-caused (some other paint in the tile's own stacking context, never identified). Rather than keep fighting it, `CachingTileLayer` now draws `L.rectangle` overlays for cache-hit tiles into a dedicated `tileCacheDebugPane` (z-index 250, above `tilePane`'s 200, `pointer-events: none` so it never intercepts taps meant for tiles/POIs below) — set up in `onAdd()`/torn down in `onRemove()`, one rectangle added per cache hit in `createTile()` and removed on that tile's `tileunload` event, keyed by `z/x/y`. Tile-to-lat/lng math lives in `src/tiles/tileBounds.ts` (`tileToBounds()`, Leaflet-free, unit-tested) since it's pure geometry unrelated to Leaflet's API. `TileFetcher.fetchTile()` returns `{ blob, cacheHit }` (not a bare `Blob`) specifically so callers can react per-request — `getStats()` is cumulative and can't tell you about any one specific tile.
- **DevTools "Cache Storage" is always origin-scoped, not area-scoped** — this is a browser platform constraint, not something this app controls. Each area still gets its own named cache (`geo-browser.tiles.<areaId>` — visible as the cache's own name/label in DevTools' Cache Storage tree, e.g. `geo-browser.tiles.debug` for the `"debug"` area), but Chrome groups every named cache for one origin under a single "Cache storage" node in the Application panel; there's no way to make the browser present them as separate top-level buckets. Confirming per-area isolation means checking each named cache's *contents*, not expecting a separate tree node per area.
...
All interactive controls are at `topright` (stacked top-to-bottom in render order). `MapLayerFlyoutControl` is session-level (`MapView`, always present); `SearchControl`, `ToolsFlyoutControl`, and `ImageOverlayWidget` are area-scoped (`CurrentAreaBundle`, only exist while a current area is attached). `ImageOverlayWidget`'s own control only becomes visually non-empty once an image is actually loaded (its adjustment toolbar) — the always-visible paste-image entry point lives in `ToolsFlyoutControl` instead, see Offline Tile Caching above:
...
ToolsFlyoutControl    (record/clear/paste-image flyout — current area only)
ImageOverlayWidget    (image adjustment toolbar — current area only, only visible once an image is loaded)
...
**Ask for approval before committing.** Leave changes staged/unstaged in the working tree and wait
for explicit confirmation — the user reviews the diff in the editor (VS Code) before it becomes a
commit, not after. This applies every time, not just once per session. Once approved, commit and
push together in the same step — the review gate is what matters, not a separate approval for each
of commit and push.

## New Tasks
- **File**: [Offline Tile Caching](tasks/tile_caching.md)
- **Status**: Testing
- **GitHub Issue**: [geo-browser#103](https://github.com/croicu/geo-browser/issues/103)
- **Key Context**: Redesigned mid-implementation from bulk area pre-fetch to record-while-browsing, after confirming (against the actual OSM policy source repo's commit history) that bulk/offline tile downloading has been explicitly prohibited for years, regardless of rate-limiting — not a recent change, and not fixable by throttling harder. Recording just flips write-through caching on for whatever tiles ordinary viewport-driven browsing already requests — OSM's own explicitly *permitted* pattern. No job, no resume. Cache is per-area (`geo-browser.tiles.<areaId>`, `getTileFetcherForArea()`) and the clear button wipes only the current area's tiles — confirmed live in DevTools' Cache Storage panel (per-area naming works; Chrome just groups all of an origin's named caches under one tree node regardless, a platform constraint). Live testing under `?debug` surfaced two real bugs in the cache-hit visual diagnostic before it actually worked: (1) it needed active recording to show at all, since a plain (non-caching) tile layer never consults the Cache API — fixed via `shouldUseCachingLayer(recording, debug)` (`src/tiles/tileCacheDecision.ts`), so `?debug` alone now reads (never writes) the cache; (2) even once the cache check was confirmed running (100% hit rate verified via `document.querySelectorAll`), a CSS `box-shadow` on the tile `<img>` itself never actually rendered on screen for reasons never root-caused — replaced with a real `L.rectangle` Leaflet layer in a dedicated pane instead of continuing to debug the CSS. 356/356 tests pass, `tsc`/build clean.
...
- **Task**: Zoom sweep during recording — [geo-browser#116](https://github.com/croicu/geo-browser/issues/116) (open, `status:postponed`)
- **Key Context**: Auto/semi-auto step through zoom levels at the current center while recording (geo-browser#103), to cache more zoom levels per stop. Deliberately deferred, not decided against: risks recreating the exact pattern the original bulk-prefetch design was killed for — OSM's policy distinguishes organic viewing from systematic/automated fetching by *pattern*, not volume, and a sweep (even a one-shot, button-triggered one) reads as the latter. Explicitly acknowledged gray-zone for personal trip use, not ruled out outright.
