...
- **Cache-first tile layer**: `CachingTileLayer` (`leafletFactories.ts`, the one file allowed to import Leaflet) overrides `createTile()` to fetch manually (not via `<img src>`) so it can check the Cache API first and, while recording, write a live fetch back into the cache. `buildTileLayer()` only constructs a `CachingTileLayer` at all when `shouldUseCachingLayer(recording, debug)` (`src/tiles/tileCacheDecision.ts`, Leaflet-free/unit-tested) says so — otherwise it's a plain, fast `L.tileLayer`, so ordinary non-debug browsing with recording off (the common case) pays zero overhead. `MapLayerFlyoutControl.setTileCacheEnabled(enabled, areaId)` only triggers a tile-layer rebuild when `shouldRebuildTileLayer()` says the *effective* caching layer would actually differ — a recording toggle always rebuilds (a `CachingTileLayer`'s `writeThrough` is fixed at construction), an area change only rebuilds while the caching layer is actually in use. Always computes the tile URL via `tiles/tileUrl.ts`'s `buildTileUrl()` (fixed first subdomain, sacrificing Leaflet's `{s}`-rotation connection-parallelism) so the same tile always maps to the same cache key regardless of how many times it's requested.
- **`?debug` reads the cache even with recording off.** `shouldUseCachingLayer(recording, debug)` returns true under `?debug` alone, so a `CachingTileLayer` is constructed with `writeThrough: false` — it checks the Cache API first (so the debug border below has something to show while just browsing an area that was recorded in an *earlier* session) but never persists a live fetch. Without this, the debug border was invisible during ordinary (non-recording) browsing regardless of how much was already cached, since a plain `L.tileLayer` never consults the Cache API at all — confirmed live (`geo-browser#103`): DevTools showed hundreds of cached entries for the current area while the map, viewed with recording off, showed zero borders.
