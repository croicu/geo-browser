...
> debug-tagged ones)" no longer holds — `"debug"` is opt-in-only. It's hidden whenever it
...
- `groupFilter === null` → include **all** areas, no filtering. *(Superseded below —
...
- [x] No query string → all non-debug areas render; `"debug"`-tagged areas are
      hidden (opt-in-only — see Superseded note in Summary above).
- [x] `?debug=1` → only areas with `"debug"` in `group` render
      (`Context.parseGroupFilter` back-compat shorthand → `groupFilter = ["debug"]`).
- [x] `?group=<name>` (not `"debug"`) → only areas with `<name>` in `group` render,
...
- [x] `?group=<name>&debug=1` → `group` wins; `Context.debug` itself stays
      ...
      area selection once `group` is present — so `"debug"` is hidden here too
